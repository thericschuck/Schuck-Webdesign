...
function AgentEditForm({
  ...
  // Der Orchestrator liest seine Tools aus der vollen, im Code registrierten Registry
  // (lib/jarvis/tools/index.ts), nicht aus agent_tools — anders als bei Sub-Agenten
  // (buildScopedRegistry, lib/jarvis/tools/subagents.ts). Eine Zuordnungs-Checkliste hier
  // würde also Zeilen anlegen, die nie gelesen werden — deshalb nur für kind='agent' gezeigt.
  ...
  async function handleSave() {
    ...
    try {
      const patchRes = await fetch(`/api/admin/jarvis/agents/${agentId}`, {
        method: 'PATCH',
      ...
      if (showToolAssignment) {
        const toolsRes = await fetch(`/api/admin/jarvis/agents/${agentId}/tools`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      fetch(`/api/admin/jarvis/agents/${agentId}`)
...
function AgentPanelBody({
  ...
  async function toggleRun(runId: string) {
    ...
    try {
      const res = await fetch(`/api/admin/jarvis/agent-runs/${runId}`)
