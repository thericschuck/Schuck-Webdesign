...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (toolIds.length > 0) {
    const { data: tools, error: toolsError } = await admin.from('tools').select('id, name, is_irreversible').in('id', toolIds)
    ...
    // Dieselbe Regel wie in buildScopedRegistry (lib/jarvis/tools/subagents.ts) — hier
    // zusätzlich an der Eingabe-Kante durchsetzen, nicht erst beim nächsten Chat-Lauf merken.
    const irreversible = (tools ?? []).filter((t) => t.is_irreversible)
    if (irreversible.length > 0) {
      return jsonError(
        `Bestätigungspflichtige Tools können keinem Sub-Agenten zugeordnet werden: ${irreversible.map((t) => t.name).join(', ')}`,
        400
      )
    }
  }
