  ...
  return (
    <div className="w-full">
      <div className="hidden sm:block">
          ...
          {STAGES.map((stage, i) => {
            ...
            return (
              <div key={stage.key} className="flex items-center flex-1 last:flex-none">
                <div
                  className={[
                    'w-9 h-9 shrink-0 rounded-full flex items-center justify-center border-2 transition-all',
                    done ? 'bg-[#1C1C1E] border-[#1C1C1E]' : 'bg-[#F7F5F0]',
                ...
                {!isLast && (
                  <div className={['flex-1 h-0.5 mx-1', done ? 'bg-[#1C1C1E]' : 'bg-[#DED7CB]'].join(' ')} />
                )}
              </div>
            )
          })}
        ...
        {/* Labels */}
        <div className="grid mt-2" style={{ gridTemplateColumns: `repeat(${STAGES.length}, 1fr)` }}>
          {STAGES.map((stage, i) => {
            const done = i < current
            const active = i === current
            return (
              <span
                ...
                className={[
                  ...
                  active ? 'text-[#1C1C1E] font-medium' : '',
