export interface SubAgentDefinition {
  ...
  label: string
  /**
   * NICHT mehr die Laufzeit-Quelle des System-Prompts — public.agents.system_prompt ist
   * das jetzt (siehe lib/jarvis/tools/subagents.ts#buildScopedRegistry). Dient nur noch als
   * Fallback, falls die DB-Spalte leer ist oder noch auf dem Seed-Platzhalter steht, sowie
   * als Referenz für die Migration, die die DB-Spalte initial befüllt. Eine Bearbeitung im
   * Cockpit (agents.system_prompt) wirkt sich NICHT auf diese Konstante aus — und umgekehrt
   * ändert eine Anpassung hier nichts an einem bereits in der DB gepflegten Prompt.
   */
  systemPrompt: string
  /**
...
const DESIGN_AGENT_PROMPT = `Du bist der Design-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Design-Aufgabe als "task" auf und reicht deine Antwort an Eric weiter.
...
const CODE_AGENT_PROMPT = `Du bist der Code-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Aufgabe als "task" auf und reicht deine Antwort an Eric weiter.
...
const SEO_AGENT_PROMPT = `Du bist der SEO-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Aufgabe als "task" auf und reicht deine Antwort an Eric weiter.
...
const CARE_AGENT_PROMPT = `Du bist der Care-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Aufgabe als "task" auf (z.B. "Monatsbericht für Kunde X") und reicht deine Antwort an Eric weiter.
...
const AKQUISE_AGENT_PROMPT = `Du bist der Akquise-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Aufgabe als "task" auf und reicht deine Antwort an Eric weiter.
...
const FINANCE_AGENT_PROMPT = `Du bist der Finanzen-Agent von JARVIS (Schuck Webdesign) — ein fokussierter Sub-Agent, kein eigenständiger Chatbot. Der Haupt-Orchestrator ruft dich mit einer konkreten Aufgabe als "task" auf und reicht deine Antwort an Eric weiter.
