...
/**
 * Direkter DB-Snapshot für die Cold-Start-Begrüßung. Läuft server-seitig VOR dem
 * eigentlichen Claude-Call, da für Todos/Kontaktanfragen noch keine Tools existieren, mit
 * denen Claude sich die Daten selbst holen könnte.
 */
...
/**
 * Per-Prompt-Kontext aus dem Wissensgraph (Semantic Search Top-20 + Traversal Tiefe 3, s.
 * lib/domain/knowledge.ts#getClientContext). Läuft best-effort — ohne OPENAI_API_KEY oder
 * bei sonstigen Fehlern liefert sie `null`, HELM arbeitet dann einfach ohne diesen Block
...
export async function getPromptContext(lastUserText: string): Promise<string | null> {
  ...
  try {
    ...
    console.error(
      '[helm] Kontext aus Wissensgraph konnte nicht geladen werden:',
